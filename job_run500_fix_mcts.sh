...
#SBATCH --job-name=fixmcts
#SBATCH --output=slurm_logs/slurm_fixmcts_%j.out
set -euo pipefail
...
CKPT_DIR="checkpoints/run500_fixmcts_$(date +%m%d_%H%M)"
mkdir -p "$CKPT_DIR"

echo "=== Launching training (S=64, single combined loss, 500 steps, no prior threshold) ==="
python -u scripts/train_replay.py \
    --G 512 --M 8 --S 64 --n_steps 500 \
    --ckpt_dir "$CKPT_DIR" \
    ...
    --single_loss \
    --skip_elo \
    > slurm_logs/train_fixmcts_$$.log 2>&1 &
TRAIN_PID=$!

echo "=== Launching monitor (full MCTS + policy-only, no value) ==="
python -u scripts/elo_monitor_continuous.py \
    --ckpt_dir "$CKPT_DIR" \
    --G 256 --M 4 --S 16 \
    --max_gap 5 --interval 30 \
    --no-value \
    > slurm_logs/monitor_fixmcts_$$.log 2>&1 &
MONITOR_PID=$!
