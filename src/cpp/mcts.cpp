...
void MCTSManager::expand_and_backup(const int* leaf_indices, int n_leaves,
    ...
    for (int i = 0; i < n_leaves; i++) {
        ...
        // Backup: values[li] is from mover's perspective (the player who
        // made the last move in the path, = edge owner at the last level).
        // Model output value at position path_length-1 is trained from
        // the mover's perspective (aligned with plr[:, path_length-1]).
