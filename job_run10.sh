...
#SBATCH --output=slurm_logs/slurm_run10_%j.out
set -euo pipefail
...
echo ""
echo "=== Training 10 steps from scratch ==="
python -u scripts/train_replay.py \
    --G 512 --M 8 --S 64 --n_steps 10 \
    --ckpt_dir checkpoints/run10 \
...
python -u scripts/eval_elo_curve.py --ckpt_dir checkpoints/run10 --G 256 --M 4 --S 16
