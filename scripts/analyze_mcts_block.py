#!/usr/bin/env python3
"""MCTS visit analysis: blocking scenarios with high S."""
...
def run_analysis(model, model_name, scenarios, S_values, c_puct=1.0):
    for sc in scenarios:
        print(f"\n{'='*65}")
        ...
        print(f"  {'S':>5s} | {'correct_visit':>14s} | top5 visits                          | time")
        print(f"  {'-'*5} | {'-'*14} | {'-'*38} | {'-'*6}")
        ...
        for S in S_values:
            ...
            print(f"  {S:5d} | {correct_visit:14.4f} | {top5_str(rp):38s} | {dt:.1f}s")
            del mgr
...
class RandomModel:
    def __init__(self, cfg):
        self.config = cfg
        self._dummy = GomokuTransformer(cfg).to(DEVICE)
...
def main():
    ...
    parser.add_argument('--S', type=int, nargs='+', default=[256, 512, 1024, 2048, 4096])
    ...
    if not args.random:
        ...
        run_analysis(model, f"step_109 (c_puct={args.c_puct})",
                     SCENARIOS, args.S, args.c_puct)

    print(f"\n{'#'*65}")
    print(f"# RANDOM BASELINE (c_puct={args.c_puct})")
    print(f"{'#'*65}")
    rnd = RandomModel(cfg)
    run_analysis(rnd, f"Random (c_puct={args.c_puct})",
                 SCENARIOS, args.S, args.c_puct)
