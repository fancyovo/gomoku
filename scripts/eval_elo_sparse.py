#!/usr/bin/env python3
"""Sparse ELO evaluation: only pairs within max_step_gap, plus noisy_uniform baseline.
Much faster than full pairwise for many checkpoints.

Usage: python scripts/eval_elo_sparse.py --ckpt_dir checkpoints/run20_lr1e4 --max_gap 4
"""
...
class NoisyUniform:
    def __init__(self, ns=0.02, cfg=None):
        self.ns = ns
        self.config = cfg
...
def main():
    ...
    parser.add_argument('--max_gap', type=int, default=4,
                        help='Only evaluate pairs with |i-j| <= max_gap')
    ...
    n_steps = len(ckpts)
    print(f"Checkpoints: {n_steps}, max_gap={args.max_gap}")
    print(f"Models: {len(names)}")

    # Build pair list
    pairs = []
    # noisy_uniform vs all
    for j in range(n_steps):
        pairs.append(('noisy_uniform', names[1 + j]))
    # step_i vs step_j where |i-j| <= max_gap
    for i in range(n_steps):
        for j in range(i + 1, min(i + 1 + args.max_gap, n_steps)):
            pairs.append((names[1 + i], names[1 + j]))

    print(f"Pairs to evaluate: {len(pairs)} (full pairwise would be {len(names)*(len(names)-1)//2})")
    ...
    for na, nb in pairs:
        ...
        # Free non-baseline models
        if na not in ('noisy_uniform',):
            del model_cache[na]
        if nb not in ('noisy_uniform',):
            del model_cache[nb]
    ...
    # Print
    print(f"\nELO Results (S={S_ELO}, max_gap={args.max_gap}, {n_steps} steps):")
    for n in names:
        r = elo.get(n, 1500)
    ...
    ax.set_xlabel('Step')
    ax.set_ylabel('ELO')
    ax.set_title(f'Replay Buffer Training (G=512, S={S_ELO}, sparse eval |i-j|≤{args.max_gap})')
    ax.legend()
