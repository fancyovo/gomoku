#!/usr/bin/env python3
"""Train with per-step ELO evaluation. No epoch limit — early stop only."""
...
# Config
G, M, S = 512, 8, 64; TRAIN_BATCH = 128; MAX_EPOCHS = 200; EARLY_STOP = 5; N_STEPS = 10
ELO_G, ELO_M, ELO_S = 128, 4, 16
CHECKPOINT_DIR, OUTPUT_DIR = "checkpoints/train_loop", "output"
CACHE_FILE = os.path.join(OUTPUT_DIR, "elo_cache.json")
...
def run_selfplay(model):
    ...
    pos_lens=np.zeros(G,dtype=np.int32); finished=np.zeros(G,dtype=bool); results=np.zeros(G,dtype=np.int32)
    for g in range(G):
    ...
    for g in range(G):
        ...
        rv=results[g]; rv=3 if rv==0 else rv
    ...
    return trajs,results,pos_lens,bw,ww,dr
...
def train_with_early_stop(model, trajectories):
    ...
    for traj in trajectories:
        ...
        for s in range(N_SYMS):
            ...
            samples.append({"positions":remap[pos],"players":plr,"mcts_policies":pol[:,inv_remap],
                           "value_targets":val,"actual_len":L})
    n=len(samples); n_train=int(n*0.8)
    idx=np.random.permutation(n); train_idx,test_idx=idx[:n_train],idx[n_train:]
...
def play_match(model_a,model_b):
    ...
    for g in range(G_):
        ...
        elif w==2: wa+=1 if not a_black[g] else 0; wb+=1 if a_black[g] else 0
        else: dr+=1
    del kva,kvb; torch.cuda.empty_cache()
    return wa,wb,dr

def run_elo_and_plot(history):
    ...
    for i,na in enumerate(names):
        for nb in names[i+1:]:
            sa=int(na.split('_')[1].split('.')[0]); sb=int(nb.split('_')[1].split('.')[0])
            key=f"{min(sa,sb)}_{max(sa,sb)}"
            if key not in cache: missing.append((na,nb,key))
    if missing:
        print(f"  [ELO] {len(missing)} missing pairs", flush=True)
        cfg=ModelConfig(d_model=128,n_layers=16,n_heads=4,d_ff=256,board_size=15)
        for na,nb,key in missing:
            print(f"    {na} vs {nb} ...", end=" ", flush=True)
            t0=time.perf_counter()
            ma=GomokuTransformer(cfg).to(DEVICE).eval()
            ma.load_state_dict(torch.load(os.path.join(CHECKPOINT_DIR,na),map_location=DEVICE))
            mb=GomokuTransformer(cfg).to(DEVICE).eval()
            mb.load_state_dict(torch.load(os.path.join(CHECKPOINT_DIR,nb),map_location=DEVICE))
            wa,wb,d=play_match(ma,mb)
            dt=time.perf_counter()-t0
            wr=wa/(wa+wb) if (wa+wb)>0 else 0.5
            cache[key]=[wa,wb,d]
            with open(CACHE_FILE,"w") as f: json.dump(cache,f)
            print(f"{wa}-{wb} WR={wr:.1%} ({dt:.0f}s)", flush=True)
    ...
    for key,val in cache.items():
        try:
            ...
            a_name=f"step_{int(s1):06d}.pt"; b_name=f"step_{int(s2):06d}.pt"
            results.append((a_name,b_name,wa+d*0.5,wb+d*0.5))
        except: pass
    if results:
        elo=compute_elo(results)
        items=sorted((int(n.split('_')[1].split('.')[0]),r) for n,r in elo.items())
        if items:
            min_elo=min(r for _,r in items); max_elo=max(r for _,r in items)
            latest_elo=items[-1][1]
            print(f"  ELO: min={min_elo:.0f} max={max_elo:.0f} latest={latest_elo:.0f}  ", flush=True)
    ...
    if n>=2:
        ...
        for key,val in cache.items():
            try:
                ...
                if wa+wb>0: wr[sti[i1],sti[i2]]=wa/(wa+wb)
            except: pass
        for ii in range(n):
            for jj in range(ii+1,n):
                if not np.isnan(wr[ii,jj]): wr[jj,ii]=1.0-wr[ii,jj]
    ...
    plt.savefig(os.path.join(OUTPUT_DIR,"analysis.png"),dpi=150); plt.close()
    if results: return elo
    return {}
...
def main():
    ...
    print(f"Steps: {N_STEPS}, G={G} M={M} S={S} (self-play), batch={TRAIN_BATCH}")
    print(f"ELO: G={ELO_G} M={ELO_M} S={ELO_S}, MAX_EPOCHS={MAX_EPOCHS} EARLY_STOP={EARLY_STOP}")
    print()
    history={"avg_len":[],"sp_time":[],"tr_time":[],"epochs":[],"test_loss":[],"black_wr":[]}
    for step in range(N_STEPS):
        t0=time.perf_counter()
        model.eval(); traj,results,pos_lens,bw,ww,dr=run_selfplay(model)
        ...
        step_time=time.perf_counter()-t0-t_tr+t_sp  # approximate
        ...
        torch.save(model.state_dict(),ckpt_path+".tmp")
        os.replace(ckpt_path+".tmp",ckpt_path)
        with open(os.path.join(OUTPUT_DIR,"train_length.json"),"w") as f: json.dump(history,f)
        # Per-step ELO
        run_elo_and_plot(history)
        sys.stdout.flush()
    print(f"\nDone. Checkpoints: {CHECKPOINT_DIR}/, Plot: {OUTPUT_DIR}/analysis.png")
