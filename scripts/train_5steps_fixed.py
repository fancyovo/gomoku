#!/usr/bin/env python3
"""Train 5 steps using fully-fixed train_loop functions, then ELO."""
...
# Import fixed training functions from train_loop
...
def play_match(model_a,model_b):
    ...
    for move in range(1,200):
        ...
        dec_pos=torch.from_numpy(new_actions).to(DEVICE); dec_plr=torch.full((len(active),),cp,dtype=torch.long,device=DEVICE)
...
def main():
    ...
    print(f"Steps: {N_STEPS}, FULLY FIXED codebase (all 6 bugs patched)")
    ...
    ax=axes[0,0]; ax.plot(history["avg_len"],"o-",color="C0",markersize=6); ax.set_title("Game Length"); ax.grid(True,alpha=0.3)
    ax=axes[0,1]; ax.plot(history["black_wr"],"o-",color="C3",markersize=6); ax.axhline(y=0.5,color="gray",linestyle="--",alpha=0.4); ax.set_title("Black WR"); ax.grid(True,alpha=0.3)
    ax=axes[1,0]
    if items: se,ra=zip(*items); ax.plot(se,ra,".-",color="C1",markersize=8,linewidth=2); ax.axhline(y=1500,color="gray",linestyle="--",alpha=0.4)
    ax.set_title(f"ELO ({ELO_G} games/pair)"); ax.grid(True,alpha=0.3)
    ax=axes[1,1]
    ...
    if n>=2:
        ...
        ax.set_xticklabels(steps_list,rotation=45,ha="right",fontsize=7); ax.set_yticklabels(steps_list,fontsize=7)
        plt.colorbar(im,ax=ax); ax.set_title("Win Rate Heatmap")
    plt.tight_layout(); plt.savefig(os.path.join(OUTPUT_DIR,"analysis_after.png"),dpi=150); plt.close()

    print(f"\nELO (FULLY FIXED):")
    for s,r in items: print(f"  Step {s}: ELO={r:.0f}")
    print(f"  ΔELO = {items[-1][1]-items[0][1]:.0f}")
    print(f"\nPlots: output/analysis_before.png (C++ fixes only), output/analysis_after.png (all fixes)")
