#!/usr/bin/env python3
"""Train 5 steps + ELO tournament + all analysis plots."""
...
# ─── Config ─────────────────────────────────────────────────────
G = 512; M = 8; S = 64; TRAIN_BATCH = 128; MAX_EPOCHS = 200; EARLY_STOP = 5
N_STEPS = 10
ELO_G = 128; ELO_M = 4; ELO_S = 16  # faster ELO: 128 games, 64 sims
...
def play_match(model_a, model_b):
    ...
    for g in range(G_):
        ...
        p0[g, first_acts[g]] = True  # first move always black
...
def main():
    ...
    print(f"Steps: {N_STEPS}, Self-play: G={G} M={M} S={S}, Train batch={TRAIN_BATCH}")
    ...
    print(f"\nTraining complete ({N_STEPS} steps). Running ELO tournament...")
    ...
    print(f"{len(ckpt_names)} checkpoints, {n_pairs} pairs to play")
    ...
    print(f"ELO cache: {len(cache)} pairs")
