#!/usr/bin/env python3
"""Train 5 steps from scratch, then ELO tournament + plots."""
...
G, M, S = 512, 8, 64; TRAIN_BATCH = 128; MAX_EPOCHS = 200; EARLY_STOP = 5; N_STEPS = 5
...
def run_selfplay(model):
    ...
    while True:
        ...
        for i,g in enumerate(active):
            ...
            pos_lens[g]+=1
        dec_pos=torch.from_numpy(new_actions).to(DEVICE); dec_plr=torch.from_numpy(new_plrs).to(DEVICE)
...
def main():
    ...
    print(f"Steps: {N_STEPS}, G={G} M={M} S={S} (self-play), MAX_EPOCHS={MAX_EPOCHS}")
    ...
    for step in range(N_STEPS):
        ...
        best_ep,best_loss=train(model,traj)
    ...
    ax=axes[0,0]; ax.plot(history["avg_len"],"o-",color="C0",markersize=6)
    ax.set_xlabel("Step"); ax.set_ylabel("Avg Game Length"); ax.set_title("Game Length"); ax.grid(True,alpha=0.3)
    ax=axes[0,1]; ax.plot(history["black_wr"],"o-",color="C3",markersize=6)
    ...
    if results:
        ...
        se,ra=zip(*items); ax.plot(se,ra,".-",color="C1",markersize=8,linewidth=2)
    ...
    plt.tight_layout(); plt.savefig(os.path.join(OUTPUT_DIR,"analysis.png"),dpi=150); plt.close()
    print(f"Plot: {OUTPUT_DIR}/analysis.png")
