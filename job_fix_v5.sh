...
#SBATCH --gres=gpu:1
#SBATCH --exclude=anode05,anode06
#SBATCH --cpus-per-task=12
#SBATCH --mem=32G
#SBATCH --time=1-00:00:00
#SBATCH --job-name=fix_v5
#SBATCH --output=slurm_logs/slurm_fix_v5_%j.out
set -euo pipefail
...
CKPT_DIR="checkpoints/run500_fix_v5_$(date +%m%d_%H%M)_$RANDOM"
mkdir -p "$CKPT_DIR"
...
python -u scripts/train_replay.py \
    ...
    --c_puct 0.2 \
    --skip_elo \
    > slurm_logs/train_fix_v5_$$.log 2>&1 &
TRAIN_PID=$!
...
python -u scripts/elo_monitor_continuous.py \
    ...
    --no-value \
    > slurm_logs/monitor_fix_v5_$$.log 2>&1 &
MONITOR_PID=$!
