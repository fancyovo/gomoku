...
#SBATCH --job-name=fix_v3
#SBATCH --output=slurm_logs/slurm_fix_v3_%j.out
set -euo pipefail
...
CKPT_DIR="checkpoints/run500_fix_v3_$(date +%m%d_%H%M)_$RANDOM"
mkdir -p "$CKPT_DIR"
...
python -u scripts/train_replay.py \
    ...
    --c_puct 1.0 \
    --skip_elo \
    > slurm_logs/train_fix_v3_$$.log 2>&1 &
TRAIN_PID=$!
...
python -u scripts/elo_monitor_continuous.py \
    ...
    --no-value \
    > slurm_logs/monitor_fix_v3_$$.log 2>&1 &
MONITOR_PID=$!
