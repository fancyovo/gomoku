...
#SBATCH --job-name=deep8
#SBATCH --output=slurm_logs/slurm_deep8_%j.out
set -euo pipefail
...
CKPT_DIR="checkpoints/run200_S64_deep8x8x8_$(date +%m%d_%H%M)"
mkdir -p "$CKPT_DIR"

echo "=== Launching training (S=64, 8+8+8) in background ==="
python -u scripts/train_replay.py \
    ...
    --from_scratch \
    --lr 1e-4 \
    --n_shared 8 --n_policy 8 --n_value 8 \
    --skip_elo \
    > slurm_logs/train_deep8_$$.log 2>&1 &
TRAIN_PID=$!

echo "=== Launching monitor (max_gap=5) in background ==="
python -u scripts/elo_monitor_continuous.py \
    --ckpt_dir "$CKPT_DIR" \
    --G 256 --M 4 --S 16 \
    --n_shared 8 --n_policy 8 --n_value 8 \
    --max_gap 5 --interval 30 \
    > slurm_logs/monitor_deep8_$$.log 2>&1 &
MONITOR_PID=$!
