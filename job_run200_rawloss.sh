...
#SBATCH --job-name=rawloss
#SBATCH --output=slurm_logs/slurm_rawloss_%j.out
set -euo pipefail
...
CKPT_DIR="checkpoints/run200_S64_rawloss_$(date +%m%d_%H%M)"
mkdir -p "$CKPT_DIR"

echo "=== Launching training (S=64, single loss, raw CE) ==="
python -u scripts/train_replay.py \
    ...
    --raw_loss \
    --skip_elo \
    > slurm_logs/train_rawloss_$$.log 2>&1 &
TRAIN_PID=$!
...
python -u scripts/elo_monitor_continuous.py \
    ...
    > slurm_logs/monitor_rawloss_$$.log 2>&1 &
MONITOR_PID=$!
